...
export default async function SearchPage({ searchParams }: Props) {
  ...
  const { data: games, total, totalPages } = q
    ? await getGames({ q, page, limit: 24 })
    ...
    <div className="section-container py-8">
      ...
      <div className="max-w-xl mx-auto mb-8">
        <h1 className="font-display text-2xl font-bold text-white mb-4 text-center">
          {q ? `Resultados para "${q}"` : 'Buscar Juegos'}
        </h1>
        <SearchBar defaultValue={q} placeholder="Buscar juegos..." size="lg" />
      </div>

      {q && (
        <p className="text-sm text-muted mb-6">
          {total > 0 ? (
            <><strong className="text-white">{total}</strong> juegos encontrados para "<strong className="text-white">{q}</strong>"</>
          ) : (
            <>No se encontraron juegos para "<strong className="text-white">{q}</strong>"</>
          )}
        </p>
      )}
      ...
      {q && total === 0 && (
        <div className="text-center py-16 text-muted">
          ...
          <p className="text-lg mb-2">No encontramos juegos para "{q}"</p>
